...
const ThreadComposer = ({ isPreview, isReply, threadId }: ThreadComposerProps) => {
	...
	const handleSubmit = async () => {
		...
		addThread({ content: threadContent, threadId, userId: userProfile?._id as Id<'users'>, mediaFiles: mediaStorageIds });
		setThreadContent('');
		setMediaFiles([]);
		router.dismiss();
	};
	...
	return (
		<TouchableOpacity
			...
			<View style={styles.topRow}>
				...
				<TouchableOpacity onPress={removeThread} style={[styles.cancelButton, { opacity: isPreview ? 0 : 1 }]}>
